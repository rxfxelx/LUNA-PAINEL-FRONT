...
async function loadChats() {
  ...
  try {
    ...
    // Classificação de TODOS (background – mantém, mas a aba ativa já terá instantânea ao abrir)
    try {
...
async function classifyInstant(chatid, items) {
  // tenta no backend (novo endpoint) e, se falhar, usa local
  try {
    ...
    if (stage) {
      const rec = setStage(chatid, stage, makeTranscriptKey(items))
...
async function loadMessages(chatid) {
  ...
  try {
    ...
    // === Classificação INSTANTÂNEA (antes de renderizar) ===
...
- Contatos é o padrão.
Guard contra mensagens automáticas de menu/cardápio.
*/
const HOT_HINTS = [
...
function classifyByRules(items) {
  ...
  for (const m of msgs) {
    const me = m.fromMe || m.fromme || m.from_me || false
    const text = norm(m.text || m.caption || m?.message?.text || m?.message?.conversation || m?.body || m?.message?.extendedTextMessage?.text || "")
    if (!text || !me) continue
